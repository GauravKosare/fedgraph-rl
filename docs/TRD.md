...
| **F‑12** | Baselines: `random`, `fraud_greedy`, `all` (fixed cohort), and `coverage` *(v0.4 — non‑learned staleness+value scheduler)*; plus a centralised (non‑federated) reference. | `HeuristicController`; `centralised_*`. |
...
| 0.4 | Coverage‑aware **non‑learned** heuristic (`HeuristicController(kind="coverage")`): staleness + value score, guarantees full coverage before any retrain | **implemented** | **Also ties random** (scarce: 0.556 ± 0.112 vs 0.573 ± 0.101). Definitive null result — nothing beats random on this problem. README §4.6, DESIGN_LOG §19. |
